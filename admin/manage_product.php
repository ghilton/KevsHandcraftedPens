...
$item = 'Product';				// table name for query
$page_child = 'add_edit_product.php';				// table name for query
...
if($_POST['Delete'] == 'Delete Selected'){
	...
	if($arr_num > 0){
		...
		while($fetch_s = mysql_fetch_assoc($exec_s)){
			@unlink('../demo_assets/large/'.$fetch_s[image]);
...
if($_GET['do'] == 'delete' && ctype_digit($_GET['id'])){
	...
	@unlink('../demo_assets/large/'.$image);
	@unlink('../demo_assets/small/'.$image);
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string_pg; ?>" method="post" name="<?php echo $item; ?>">
          ...
          <table width="100%" cellspacing="0" cellpadding="0"  class="maintbl" >
            ...
            <tr class="scndrow <?php echo $z%2 ? '' : 'alternate';?>">
              ...
              <td align="left"><div class="n_1"><?php echo $fetch['name']; ?></div></td>
              <td align="center"><a href="sort_gen.php?type=name&tab=<?php echo $tabl; ?>&height=500&width=600&rel=<?php echo $fetch['id']; ?>" class="thickbox"><img src="images/sort-neither.png" title="Sort" width="13" height="16" /></a></td>
