...
$item = 'Categories';				// table name for query
$page_child = 'add_edit_category.php';				// table name for query
$namep = $_SERVER['PHP_SELF'];					// Page name
$query_string = array("search" => "$search_txt", "type" => '1');
...
$select = "$base_tabl.* ";
$from = "  $base_tabl where type='1' AND archived='0' ";
...
if($_POST['Delete'] == 'Delete Selected'){
	...
	if($arr_num > 0){
		...
		$sql = "UPDATE $base_tabl set archived='1' AND status='1' where id in (";
		$j = 0;
		while($j<$arr_num){
...
if($_GET['do'] == 'delete' && ctype_digit($_GET['id'])){
	...
	$sql = "UPDATE $base_tabl set archived='1' AND status='1' where id='$id'"; 
	if(mysql_query($sql)){
		$flag['d'] = $item;
	}
	else{
    ...
    <div class="content">
      ...
      <div class="down_category" style="margin-top:30px">
        <div class="head"> <span class="head_text"><?php echo $item; ?></span> 
                  <div class="add" style="margin-left:20px; margin-top:9px;">
<!--            <form method="GET" name="sort" id="sort" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
              Sort By:
              <select name="sort" onchange="document.sort.submit();">
                ...
                <option value="citizenship" <?php if($sort == "citizenship"){ echo 'selected="selected"'; } ?> >Citizenship</option>
              </select>
            </form>
-->          </div>
<div class="add"> <a href="<?php echo $page_child; ?>?type=1"> <span class="add1"> <span class="head_text1"> <?php echo $item; ?></span> </span> </a> </div>
        ...
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string_pg; ?>" method="post" name="<?php echo $item; ?>">
          ...
          <table width="100%" cellspacing="0" cellpadding="0"  class="maintbl" >
            <tr class="fstrow"  >
              ...
              <!--<td align="left" width="30%" style="padding-left:10px"> Name </td>-->
              <td align="left" width="20%"> Name </td>
              <td align="center" width="30%"> Position </td>
              <td align="center" width="20%"> Displayed </td>
              <td align="center" width="30%"> Actions </td>
            </tr>
            <?php
                $exec = mysql_query($sql2) or die(mysql_error());
				if(mysql_num_rows($exec) == 0){
					echo '<tr><td colspan="5" align="center"><div style="margin-top:10px;">No result Found</div></td></tr>';
				}
                $z = 0;
            ...
            <tr class="scndrow <?php echo $z%2 ? '' : 'alternate';?>">
              ...
              <td align="left"><?php echo $fetch['name']; ?></td>
               <td align="center"><a href="sort_gen.php?categ=1&type=name&tab=<?php echo $tabl; ?>&height=500&width=600&rel=<?php echo $fetch['id']; ?>" class="thickbox"><img src="images/sort-neither.png" title="Sort" width="13" height="16" /></a></td>
              <td align="center"><?php if($fetch['status'] == '1'){ ?>
                ...
                <?php } ?></td>
              <td align="center"><a style="margin:0px 10px;" href="view_category.php?id=<?php echo $fetch['id'].$q_string_pg; ?>" title="View"> <img src="images/view.png" /> </a> <a style="margin:0px 10px;" href="<?php echo $page_child; ?>?do=edit&id=<?php echo $fetch['id'].$q_string_pg; ?>" title="Edit"> <img src="images/edit.png" /> </a> <a style="margin:0px 10px;" href="<?php echo $namep.'?do=delete&id='.$fetch['id'].$q_string_pg; ?>" title="Delete" onclick="return del_fun();"> <img src="images/delete.png" /> </a></td>
            </tr>
            <?php $z++; } ?>
          </table>
